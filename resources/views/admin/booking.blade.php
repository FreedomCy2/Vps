                    ...
                    <tbody id="booking-rows" class="bg-white divide-y divide-gray-200">
                        @forelse($user_bookings as $user_booking)
                            <tr data-id="{{ $user_booking->id }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">{{ \Carbon\Carbon::parse($user_booking->date . ' ' . $user_booking->time)->format('h:i A') }}</td>
            ...
            function attachDeleteListeners(){
                document.querySelectorAll('.delete-booking').forEach(btn => {
                    ...
                    btn._delHandler = function(){
                        ...
                        if (!csrfToken) {
                            alert('CSRF token not found. Please add <meta name="csrf-token" content="{{ csrf_token() }}"> to your layout head section.');
                            return;
                        }
